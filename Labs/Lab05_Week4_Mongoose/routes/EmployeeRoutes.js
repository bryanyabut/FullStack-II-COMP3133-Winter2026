  ...
  if (Object.keys(req.query).length != 2) {
    res.send(
      JSON.stringify({
        status: false,
        message:
          "Insufficient query parameters. Must provide firstname and lastname both",
      }),
    );
  } else {
    ...
    //alternatively - check for query parameter values; cannot be empty or certain types
...
app.post("/employee", async (req, res) => {
  try {
    ...
    if (err?.name === "ValidationError") {
      ...
      // const emailMsg = err.errors?.email?.message;
